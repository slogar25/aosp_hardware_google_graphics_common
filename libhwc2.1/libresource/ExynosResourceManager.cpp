        ...
        for (uint32_t j = 0; j < mOtfMPPs.size(); j++) {
            if ((ret = mOtfMPPs[j]->isSupported(*display, src_img, dst_img)) == NO_ERROR) {
                ...
                if (((-ret) == eMPPUnsupportedFormat) &&
                    ...
                    layer->mSupportedMPPFlag |= mOtfMPPs[j]->mLogicalType;
                    HDEBUGLOGD(eDebugResourceManager, "\t%s: supported with yuv dst", mOtfMPPs[j]->mName.c_str());
                }
            }
            if (ret < 0) {
                HDEBUGLOGD(eDebugResourceManager, "\t%s: unsupported flag(0x%" PRIx64 ")", mOtfMPPs[j]->mName.c_str(), -ret);
        ...
        for (uint32_t j = 0; j < mM2mMPPs.size(); j++) {
            if ((ret = mM2mMPPs[j]->isSupported(*display, src_img, dst_img)) == NO_ERROR) {
                ...
                if (((-ret) == eMPPUnsupportedFormat) &&
                    ...
                    layer->mSupportedMPPFlag |= mM2mMPPs[j]->mLogicalType;
                    HDEBUGLOGD(eDebugResourceManager, "\t%s: supported with yuv dst", mM2mMPPs[j]->mName.c_str());
                }
            }
            if (ret < 0) {
                HDEBUGLOGD(eDebugResourceManager, "\t%s: unsupported flag(0x%" PRIx64 ")", mM2mMPPs[j]->mName.c_str(), -ret);
                uint64_t checkFlag = 0x0;
...
int32_t ExynosResourceManager::preAssignResources()
{
    ...
    for (uint32_t i = 0; i < mOtfMPPs.size(); i++) {
        ...
        if (mOtfMPPs[i]->mPreAssignDisplayList[displayMode] != 0) {
            HDEBUGLOGD(eDebugResourceManager, "\t%s check, dispMode(%d), 0x%8x", mOtfMPPs[i]->mName.c_str(), displayMode, mOtfMPPs[i]->mPreAssignDisplayList[displayMode]);
    ...
    for (uint32_t i = 0; i < mM2mMPPs.size(); i++) {
        ...
        HDEBUGLOGD(eDebugResourceManager, "\t%s check, 0x%8x", mM2mMPPs[i]->mName.c_str(), mM2mMPPs[i]->mPreAssignDisplayList[displayMode]);
